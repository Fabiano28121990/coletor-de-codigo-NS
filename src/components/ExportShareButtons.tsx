    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
      <div className="flex flex-wrap gap-3">
        ...
        <div className="relative">
          ...
          {showShareMenu && (
            <div className={`absolute ${isMobile ? 'inset-0 fixed' : 'top-full mt-2 left-0'} bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg shadow-lg z-10 ${isMobile ? 'w-full h-full flex items-end' : 'min-w-[250px]'}`}>
              {isMobile && (
                <div className="w-full bg-white dark:bg-slate-700 rounded-t-2xl shadow-2xl">
                  <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-600">
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-white">Compartilhar</h3>
                    <button
                  ...
                  <div className="grid grid-cols-2 gap-3 p-4">
                    <button
                      onClick={() => handleShareClick(() => shareViaWhatsApp(items, listName))}
                      className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-lg transition text-slate-700 dark:text-slate-200"
                    >
                    ...
                    <button
                      onClick={() => handleShareClick(() => shareViaEmail(items, listName))}
                      className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-lg transition text-slate-700 dark:text-slate-200"
                    >
                    ...
                    <button
                      onClick={() => handleShareClick(() => shareToGoogleDrive(items, listName))}
                      className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-lg transition text-slate-700 dark:text-slate-200"
                    >
                      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    <button
                      onClick={() => handleShareClick(() => shareToOneDrive(items, listName))}
                      className="flex flex-col items-center gap-2 p-4 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-lg transition text-slate-700 dark:text-slate-200"
                    >
                      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="currentColor">
